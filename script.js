import * as THREE from 'https://unpkg.com/three@0.148.0/build/three.module.js';
    ...
    window.goHome = function() {
      ...
      if (renderer && renderer.xr && renderer.xr.getSession()) {
        mostrarNotificacao('🏠 Voltando ao início...', 1500);
    ...
    function processQRCode(qrData) {
      ...
      if (qrData.length > 3) {
        ...
        mostrarNotificacao('🎯 QR Code detectado!', 1500);
    ...
    function definirPontoReferencia(qrData) {
      ...
      // INICIAR AR AUTOMATICAMENTE após calibração
      setTimeout(() => {
        iniciarARAutomaticamente();
      }, 500); // Pequeno delay para UI se atualizar
    }

    function iniciarARAutomaticamente() {
      // Procurar o botão AR criado pelo ARButton
      ...
      if (arButton) {
        ...
        // Fallback: procurar qualquer botão que contenha AR
        const buttons = document.querySelectorAll('button');
        const arFallback = Array.from(buttons).find(btn => 
          btn.textContent.toLowerCase().includes('ar') ||
          btn.textContent.toLowerCase().includes('enter')
        );
        
        if (arFallback) {
          console.log('Usando botão AR fallback...');
          arFallback.click();
        } else {
          console.warn('Botão AR não encontrado');
          alert(`Calibração realizada!\nEvento: ${pontoReferencia.qrCode}\nClique no botão "Enter AR" para ${currentMode === 'admin' ? 'criar pontos' : 'visualizar pontos'}.`);
        }
      }
    }
    ...
    function updateCalibrationUI() {
      ...
      if (calibrado) {
        ...
        if (isUser) {
          ...
          instructionsEl.innerHTML = 
            `<strong>Evento:</strong> ${pontoReferencia?.qrCode}<br>
             ...
             <strong>Status:</strong> AR iniciado automaticamente<br>
             <em>Os pontos aparecerão na tela em alguns segundos</em>`;
        } else {
          instructionsEl.innerHTML = 
            `<strong>QR:</strong> ${pontoReferencia?.qrCode}<br>
             <strong>Status:</strong> AR iniciado automaticamente<br>
             <em>Toque no retículo verde para criar pontos</em>`;
        }
      } else {
        ...
        instructionsEl.innerHTML = isUser ?
          '1. Calibre com o QR Code do evento<br>2. AR iniciará automaticamente' :
          '1. Calibre com o QR Code<br>2. AR iniciará automaticamente';
      }
    }
    ...
    function onSessionStart(){
      ...
      // Mostrar feedback visual de que AR iniciou após calibração
      if (calibrado && pontoReferencia) {
        mostrarNotificacao(`✅ AR ativo - Evento: ${pontoReferencia.qrCode}`, 3000);
      }
      ...
      session.requestReferenceSpace('local').then(function(refSpace){
        ...
        if (calibrado && pontoReferencia) {
          // CRÍTICO: Definir ponto de referência IMEDIATAMENTE ao iniciar AR
          if (!pontoReferencia.arPosition) {
            pontoReferencia.arPosition = new THREE.Vector3(0, 0, 0);
            console.log('Ponto de referência AR definido no início da sessão');
          }
          
          // Aguardar um pouco para tracking estabilizar, então carregar pontos
          setTimeout(() => {
            ...
            if (currentMode === 'user' && pontos.length > 0) {
              mostrarNotificacao(`📍 ${pontos.length} pontos carregados`, 2000);
            } else if (currentMode === 'admin') {
              mostrarNotificacao(`🔧 Modo criação ativo`, 2000);
            }
          }, 1500); // Maior delay para garantir estabilidade
        }
      });
    }
